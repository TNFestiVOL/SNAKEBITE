        ...
        <div className="mt-2 text-xs">
            <button
                ...
                className={cn(
                    ...
                    expanded ? "bg-slate-800 border-slate-600" : "bg-slate-800/30 border-slate-700"
                )}
            >
...
export default function MessageBubble({ message }) {
    ...
    return (
        <div className={cn("flex gap-3", isUser ? "justify-end" : "justify-start")}>
            {!isUser && (
                <div className="h-8 w-8 rounded-lg bg-emerald-500/10 flex items-center justify-center mt-0.5 flex-shrink-0">
            ...
            <div className={cn("max-w-[85%]", isUser && "flex flex-col items-end")}>
                {message.content && (
                    <div className={cn(
                        ...
                        isUser ? "bg-emerald-600 text-white" : "bg-slate-800 border border-slate-700"
                    )}>
                        {isUser ? (
                            ...
                            <ReactMarkdown 
                                className={cn(
                                    ...
                                    "prose-a:text-emerald-400 prose-li:text-slate-300",
                                ...
                                components={{
                                    code: ({ inline, className, children, ...props }) => {
                                        ...
                                        return !inline && match ? (
                                            ...
                                            <code className="px-1 py-0.5 rounded bg-slate-900 text-emerald-400 text-xs">
                                                {children}
                                            </code>
                                        );
                                    },
                                    a: ({ children, ...props }) => (
                                        <a {...props} target="_blank" rel="noopener noreferrer" className="text-emerald-400 hover:underline">{children}</a>
                                    ),
                                    p: ({ children }) => <p className="my-1 leading-relaxed text-slate-300">{children}</p>,
